...
def import_data(filepath):
    ...
    return df[['longitude', 'latitude', 'hashtags']]
...
if __name__ == "__main__":
    ...
    ms_values = ms_data.ix[:, 0:2].values
    ...
    cluster_centers = cluster_centers[(cluster_centers['longitude'] < 4.908976) & (cluster_centers['longitude'] > 4.802508)
        & (cluster_centers['latitude'] < 45.793688) & (cluster_centers['latitude'] > 45.715569)]
    ...
    for k in range(n_centers_):
        try:
            ...
            if len(cluster_points) < 30:
                print "Removing cluster {} ({} points)".format(k, len(cluster_points))
                ms_data = ms_data[(ms_data['cluster'] != k)]
                cluster_centers = cluster_centers[(cluster_centers['cluster'] != k)]
        except:
            print "Removing cluster {} (no points)".format(k)
            cluster_centers = cluster_centers[(cluster_centers['cluster'] != k)]
